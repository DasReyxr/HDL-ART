...
architecture juve3dstudio of b2f is
    constant Decimal : integer := 10000;
    constant precision : integer := 41; -- Precision of the fractional part
    signal counter_s, counter_a : unsigned(5 downto 0) := (others => '0');
    signal enable : std_logic := '0';
    signal Val_s, Val_a : unsigned(39 downto 0) := (others => '0');
    signal result, result_s : unsigned(39 downto 0) := (others => '0');
    signal stages_a, stages_s : unsigned(4 downto 0) := (others => '0');
    signal decimal_unsigned : unsigned(39 downto 0) := (others => '0');
...
begin
    ...
    decimal_unsigned <= to_unsigned(Decimal, 40);
    ...
    Val_a <= unsigned((25 downto 0 => '0')   &Val) when counter_s = 0  else
            (Val_s(39-1 downto 0)& '0' ) - decimal_unsigned when 2*Val_s >= decimal_unsigned and enable = '1' and stages_a = stage_mult else
            Val_s(39-1 downto 0) & '0' when  enable = '1' and stages_a = stage_mult else
            Val_s;
    ...
    result<= (others => '0')  when rst ='1' else
            result_s(38 downto 0) & '1' when Val_s(38 downto 0) & '0' >= decimal_unsigned and stages_a = stage_assign and enable = '1' else 
            result_s(38 downto 0) & '0' when stages_a = stage_assign and enable = '1' else
            result_s;
